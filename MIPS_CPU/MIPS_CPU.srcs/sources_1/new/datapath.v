...
module datapath #(parameter WIDTH=32, ADDR=16, REGBITS=5, CONST_ZERO = 32'b0, CONST_ONE = 32'b100)(
    ...
    //寄存器组写入数据reg_write_data二路选择器
    mux2 #(WIDTH) reg_write_data_mux(alu_out,mem_data,reg_write_sel,reg_write_data);
    ...
    assign offset16 = instr[ADDR-1]?{16'b1111111111111111,instr[ADDR-1:0]}:{16'b0,instr[ADDR-1:0]};  //lw、sw offset、i型指令imme（符号拓展）
    assign offsetu = {16'b0,instr[ADDR-1:0]};  //i型指令imme（无符号拓展）
    assign offseth = {instr[ADDR-1:0],16'b0};  //i型指令imme（左移16位，低位补0）
    //alu计算数来源1四路选择器
